...
public class RTFile {
    ...
    /**
     * 读取json字符串文件为任意类型的对象返回
     *
     * @param FilePath
     * @param class_type 要转换的类型
     * @return 任意指定的对象类型
     */
    public static Object ReadJsonFileToOtherTypeObject(String FilePath, Class class_type) {
        String File_data = Read(FilePath);
        //读取json文本内容
        Gson json = new Gson();
        Object back = json.fromJson(File_data, class_type);
        if (back == null) {
    ...
    /**
     * 读取json文本字符转为json再讲指定的key值转为指定的对象
     *
     * @param FilePath
     * @param Key
     * @param change_type 任何类型的class
     * @return
     */
    public static Object ReadJsonFileKeyToOther(String FilePath, String Key, Class change_type) {

        //先转化为json对象
        JsonObject json = (JsonObject) ReadJsonFileToOtherTypeObject(FilePath, JsonObject.class);
        //把其中一个值读为string
        String key_json = json.get(Key).toString();
        //再进行转换为指定的类型
        Gson use_gson = new Gson();
        Object back = use_gson.fromJson(key_json, change_type);
        if (back == null) {
            return new HashMap();
        }
        return back;
